...
class Commands():
    ...
    async def change_user_nickname(self, get_user, nickname, guild_id=None): # WORKING ON IT
    ...
    async def add_emoji(self, emoji_name, emoji_url=None):
        if self.command == 'addemoji':
            ...
            try:
                ...
                if path.isfile("Archives/emoji.png"):
                    remove("Archives/emoji.png")

                if not emoji_url:
                    emoji_url = self.context.attachments[0].url

                with open('Archives/emoji.png', 'wb') as emoji_handler:
                    emoji_handler.write(get(emoji_url).content)
                    emoji_handler.close()

                with open('Archives/emoji.png', 'rb') as emoji_image:
    ...
    async def gentalha(self, content):
        if content.lower() == 'vamos morangs! não se junte com essa gentalha!':
            ...
            if self.author.id == DEVELOPER_ID:
                ...
                randomize_msg = choice(('Você não é meu papai...', 'PARA!! VOCÊ NÃO É MEU PAPAI!!'))
